...
/**
 _db=nil，本质上只是给了数据库一个名字，并没有真实创建或者获取数据库
 */
...
- (BOOL)open {
    ...
    //err 为0代表数据打开成功，打开数据库的主要代码是下面这句
...
- (BOOL)close {
    [self clearCachedStatements]; // 清除缓存
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {// 数据库被使用或者被锁
        ...
        }else if (SQLITE_OK != rc) { // 其他情况
            NSLog(@"error closing!: %d", rc);
        }
    } while (retry); // 当rc=SQLITE_OK
    _db = nil;
    return YES;
}
...
#pragma mark 执行缓存sql语句的操作操作
- (void)setCachedStatement:(FMStatement*)statement forQuery:(NSString*)query {
    
    query = [query copy]; // in case we got handed in a mutable string...
    [statement setQuery:query];
    
    // 缓存中以sql语句作为key，获取对象，如果对象存在添加到NSMutableSet中；为什么用NSMutableSet来接收，解释下；1、只能存放对象类型，2、对象是不重复的，3、对象是无序的；
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    if (!statements) {
    ...
    // 缓存以sql为key，NSMutableSet类型为value，存储到可变字典中；
    [_cachedStatements setObject:statements forKey:query];
    
    FMDBRelease(query);
}

#pragma mark - Key routines FMDB的加解密
...
//执行SQL语句
//sql : insert into student(studentId,sName,sAge) values(?,?,?)   22 小二 2
#pragma mark - SQL manipulation
/**

 @param obj 22
 @param idx 1
 @param pStmt 0x0000000100364a00
 */
...
- (void)bindObject:(id)obj toColumn:(int)idx inStatement:(sqlite3_stmt*)pStmt {
    ...
    else if ([obj isKindOfClass:[NSData class]]) { // NSDate 类型
    ...
    else if ([obj isKindOfClass:[NSDate class]]) {   // NSDate 类型、text 、double
        if (self.hasDateFormatter)
            sqlite3_bind_text(pStmt, idx, [[self stringFromDate:obj] UTF8String], -1, SQLITE_STATIC);
        else
            sqlite3_bind_double(pStmt, idx, [obj timeIntervalSince1970]);
    }
    else if ([obj isKindOfClass:[NSNumber class]]) { // NSNumber 类型
...
#pragma mark - Execute queries 查询数据库
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_shouldCacheStatements) {
        statement = [self cachedStatementForQuery:sql];// 获取到缓存中的prepared语句
    ...
    //对dictionaryArgs参数的处理，类似于":age"参数形式
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            // Get the index for the parameter name.  //得到指定参数的名称，就是需要更新的参数名字
            int namedIdx = sqlite3_bind_parameter_index(pStmt, [parameterName UTF8String]);
            
            FMDBRelease(parameterName);
            // *  将通配符？:age按照索引 赋值为obj。
            //*  SELECT * FROM t_student WHERE age > :age --> SELECT * FROM t_student WHERE age > obj
            if (namedIdx > 0) {
        ...
        while (idx < queryCount) {
            ...
            idx++;
            // *  将通配符？:age按照索引 赋值为obj。
            //*  SELECT * FROM t_student WHERE age > ？ --> SELECT * FROM t_student WHERE age > obj
    ...
    //6、释放 statement对象，让后缓存
    
    //释放，执行完sqlite语句就可以释放了，释放后它将为空，
    FMDBRetain(statement); // to balance the release below
    //生成FMStatement对象
    if (!statement) {  // statement不为空，进行缓存
        statement = [[FMStatement alloc] init];
        [statement setStatement:pStmt];
        //缓存处理，key为sql语句，值为statement
...
#pragma mark - Execute updates 更新数据库
#pragma mark  解释下面这个类
/**

 @param sql SQL语句
 @param outErr 错误提示NSError
 @param arrayArgs 数组
 @param dictionaryArgs 字典
 @param args
 @return yes or no
 */
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    FMStatement *cachedStmt  = 0x00; // 用于记录缓存语句的封装
    
    if (_traceExecution && sql) { // 执行过程中是否有执行
    ...
    if (!pStmt) {
        // rc 是int类型
        rc = sqlite3_prepare_v2(_db, [sql UTF8String], -1, &pStmt, 0);
        // SQLITE_OK 可以进去看一下，如果准备语句获取失败，以下是对错误的提示的分析；
        if (SQLITE_OK != rc) {
            if (_logsErrors) {       // 打印log日志
                NSLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                NSLog(@"DB Query: %@", sql);
                NSLog(@"DB Path: %@", _databasePath);
            }
            
            if (_crashOnErrors) {    // 崩溃错误
                NSAssert(false, @"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                abort();
            }
            ...
            sqlite3_finalize(pStmt);      // 销毁这个对象
            
            _isExecutingStatement = NO;   //数据库交出使用权
            return NO;                    //查询失败，结束
        }
    }
    // sql 语句转换成sqlite成功，执行以下方法
    id obj;
    int idx = 0;
    int queryCount = sqlite3_bind_parameter_count(pStmt); //参数的个数
    
    // If dictionaryArgs is passed in, that means we are using sqlite's named parameter support
    if (dictionaryArgs) {
        
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            //遍历 参数名 dictionaryKey
            // Prefix the key with a colon.
            NSString *parameterName = [[NSString alloc] initWithFormat:@":%@", dictionaryKey];
        ...
        while (idx < queryCount) { // 遍历每一位参数
            
            if (arrayArgs && idx < (int)[arrayArgs count]) {
                obj = [arrayArgs objectAtIndex:(NSUInteger)idx];
            } else if (args) {
                obj = va_arg(args, id);//取对象
            } else {
                //We ran out of arguments
                break;
            }
            ...
            idx++;
            // obj 表示值， idx表示第几位；pStmt表示sqlite3_stmt对象
            [self bindObject:obj toColumn:idx inStatement:pStmt]; // 然后把值 赋给 参数
        }
    }
     ...
     */
    // 像更新、删除、查询，这些语句是不需要返回结果集的，所以只执行一次；
    rc = sqlite3_step(pStmt); // 执行编译后的SQL语句，执行一行停止；
    
    if (SQLITE_DONE == rc) {// 执行成功，结束
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        ...
        [self setCachedStatement:cachedStmt forQuery:sql];// 执行缓存
...
#pragma mark withFormat - 格式的更新
// *  将format的sql语句 改为 可用的sql语句
