...
@interface FMDatabase : NSObject  {
    ...
    BOOL                _isExecutingStatement;   //判断数据库是不是在使用中
    BOOL                _inTransaction;
    NSTimeInterval      _maxBusyRetryTimeInterval;
    NSTimeInterval      _startBusyRetryTime;
    
    NSMutableDictionary *_cachedStatements; //存放缓存的Statement语句
...
/** 这是一个SQLite sqlite3_stmt的包装器。
    通常使用FMDB时您不需要直接与交互“FMStatement”,而是用“< FMDatabase >”和“< FMResultSet >”。
    所以他相当于是SQL转换成sqlite
*/

@interface FMStatement : NSObject {
    void *_statement;  // SQLite
    NSString *_query;  // sql语句
    long _useCount;    // sql语句被使用的次数
    BOOL _inUse;       // 当前是否在使用
}
