...
//        execute(testKVCKeysAndValues, @"测试KVC");

        return UIApplicationMain(argc, argv, nil, NSStringFromClass([AppDelegate class]));
    }
}

void testKVCKeysAndValues() {
    id nilObj  = nil;
    NSDictionary *dict = @{
                           @"name" : @"Jack",
                           @"phone" : [NSNull null],
...
//    NSDictionary *dict = [NSJSONSerialization JSONObjectWithData:[@"{\"name\":\"Jack\",\"phone\":null,\"email\":\"222323\"}" dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:nil];
    AATestModel *model = [[AATestModel alloc] init];
    [model setValuesForKeysWithDictionary:dict];
//    model.phone = [NSNull null];
    NSLog(@"==========%@",[[dict objectForKey:@"phone"] class]);
    MJExtensionLog(@"name=%@, emain = %@， phone = %@", model.name, model.email, [model.phone class]);
    NSLog(@"%ld", model.phone.length);
}
